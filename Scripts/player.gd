...
# Player movement + draining flashlight “battery” that scales the beam,
# only active while holding the B key.
...
@export var max_energy: float            = 100.0    # full battery
@export var depletion_rate: float        = 10.0     # units drained per second
...
func _ready() -> void:
	...
	# Grab half the beam’s texture width (pivot at center)
	var tex = flashlight.texture as Texture2D
	half_tex_width = tex.get_width() * 0.5

# ── Physics (movement + aim) ───────────────────────────────────────────────────
func _physics_process(_delta: float) -> void:
	# Movement & animation (unchanged) …
...
func _process(delta: float) -> void:
	if Input.is_key_pressed(KEY_B) and energy > 0.0:
		...
		# Button released or no energy → turn off
		flashlight.enabled = false
